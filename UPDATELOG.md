...
- Fixed profile switching freezes caused by races between profile import/edit and reactivation.
- Added a hard timeout for profile scripts during config generation to avoid infinite hangs and "not responding" windows.

Windows Autostart

- Reworked Windows auto-launch method to start faster and more reliably, with a clearer switch between startup mechanisms and a copyable autostart command.

UI & Misc

- Sidebar now collapses by default for a cleaner layout; minor layout and sidebar tweaks.
- Internal refactors and CI tweaks ahead of future releases.
